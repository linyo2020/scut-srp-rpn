...
Arc::Arc(QGraphicsItem * item1, QString id1,QGraphicsItem * item2,
             QString id2, QPainterPath arc_path, const QString &id,int w,QString e)
{
    sourceItem = item1;
    targetItem = item2;
    setPath(arc_path);
...
QString Arc::getExpression() const
{
    return expression;
}

void Arc::setExpression(QString e)
{
    expression = e;
}

void Arc::f_setExperssion(QString str)
{
    m_experssion=str;
}

QString Arc::f_getExperssion() const
{
    return m_experssion;
}

void Arc::f_setLabel_1()
{
    QVariant v(weight);
    label->setText( QString("%1").arg(v.toString()) );
}

void Arc::f_setLabel_2()
{
   QString lb;
    QVariant v(weight);

        lb= QString("%1").arg(v.toString())+ QString("\n"+f_getExperssion());
